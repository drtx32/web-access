...
| CDP Proxy 浏览器操作 | 直连用户日常浏览器（Chrome / Edge / Chromium 系）或已知 profile 的 `cdp_url`，天然携带登录态，支持动态页面、交互操作、视频截帧 |
...
- **直连 profile CDP** — 如果你已经通过 CloakBrowser Manager MCP 启动了 profile，可以直接把返回的 `cdp_url` 传给 `--cdp-url`，或写入 `WEB_ACCESS_CDP_URL`，跳过浏览器发现流程
...
# 直连已知 profile 的 CDP discovery / websocket URL
WEB_ACCESS_CDP_URL=http://127.0.0.1:5100/json/list
```

合法值：`chrome` / `edge`

当你已经有一个 profile 的 `cdp_url` 时，优先使用它直连，尤其适合 CloakBrowser Manager MCP 这种“先 launch，再连接浏览器”的工作流。
